...
// A single trade note pulled from a history line, with the context needed to label it.
interface NoteEntry {
    date: string
    action: string
    symbol: string
...
// is parsed as a unit to preserve spaces; the leading tokens before it yield the sim date/symbol.
function parseNoteEntry(line: string): NoteEntry | null {
    ...
    const [, action = '', ...rest] = line.slice(0, markerIndex).split(' ')
    ...
    return { date: fields.sim ?? '', action, symbol: fields.stock ?? '', note }
}
            ...
            {notes.length === 0 ? (
                ...
                <div className="notesFeed">
                    {notes.map((entry, index) => (
                        <div className="noteItem" key={`${entry.date}-${index}`}>
