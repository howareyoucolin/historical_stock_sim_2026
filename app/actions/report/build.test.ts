...
async function testBuildSimulationReport(): Promise<void> {
    ...
    assert.equal(result.report.positions.asOfDate, '2020-12-31')
    assert.equal(result.report.positions.rows.length, 2)
    assert.equal(result.report.positions.rows[0]?.stockCode, 'AAPL')
    assert.equal(result.report.positions.rows[1]?.stockCode, 'MSFT')
    assert.equal('lots' in result.report.positions.rows[0], false)
